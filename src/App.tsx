...
import { getCryptoData, getNews, analyzeMarket } from './services/api';
...
function App() {
  ...
  useEffect(() => {
    if (cryptoData) {
      setHistoricalData(prev => [...prev, { ...cryptoData, timestamp: new Date().toISOString() }].slice(-50));
    }
  }, [cryptoData]);

  if (isLoadingCrypto || isLoadingNews || isLoadingSignal) {
    return (
  ...
  if (cryptoError || newsError || signalError) {
    return (
      <div className="min-h-screen bg-gray-100 flex items-center justify-center">
